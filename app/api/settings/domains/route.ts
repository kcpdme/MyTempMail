...
import { addResendDomain, refreshResendDomain } from "@/lib/resend";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const domain = isMockMode() || !settings.resendApiKey
      ? { name, status: "mock" as const }
      : await addResendDomain(settings.resendApiKey, name);
